...
public class FruitAndVegetable implements Item {
    ...
    public ArrayList<GameTile> getMatchingGiantSquare(GameTile tile) {
        ...
        if (!(baseItem instanceof FruitAndVegetable)) return matchingGroup;
        FruitAndVegetable basePlant = (FruitAndVegetable) baseItem;
            ...
            for (int dx = 0; dx <= 1; dx++) {
                for (int dy = 0; dy <= 1; dy++) {
                    ...
                    if (checkTile == null || (item = checkTile.getItemOnTile()) == null ||
                            !(item instanceof FruitAndVegetable) ||
                        ((FruitAndVegetable)item).isGiant() ||
                            !((FruitAndVegetable)item).getName().equals(targetName)) {
                        validSquare = false;
                        break;
                    }
